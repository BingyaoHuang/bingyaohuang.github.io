...
![setup](../images/calibration/setup.jpg)

In this case, neither a printed checkerboard pattern or a large room is needed. We can calibrate the system by projecting a checkerboard pattern to a white flat wall,  then move the whole Kinect-projector pair together to capture at least 3 shots from different position/orientations for [Zhang's method][5].
...
where $\mathbf{q}_i = [ u_i, v_i ]$ is the 2D coordinate of the $i^{th}$ checkerboard corner in projector image space, *N = `boardSize.width`\*`boardSize.height`*. *The code is inspired by [Haris][3]*
...
```
The generated checkerboard image is shown below:

![checkerboard](../images/calibration/checkerboard.png)

Then we project this image to a white flat wall using the projector. In the meanwhile, we use the Kinect to capture a depth and a color frame of the projected checkerboard pattern. 
...
If we draw the eigenvectors of $\mathbf{P}^{3d}$'s covariance matrix, what are the directions of the three eigenvectors? 

![eigenvectors](../images/calibration/eigenvectors.gif)

The first two must be on the checkerboard plane and the third is the normal of the plane! Let us recall **eigen decomposition**: the covariance matrix of $\mathbf{P}^{3d}$ is given by:
